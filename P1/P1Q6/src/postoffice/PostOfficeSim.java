...
//                Logger.getLogger(PostOfficeSim.class.getName(), null).log(Level.ERROR, null, ex);
//            }
        }
    ...
    private void announceNumber(Customer s) {
        ...
        try {
            for (int i = 0; i < numStr.length(); ++i) {
                ...
                audioClips.get(num).play(); //play number
            }

            Thread.sleep(sleepTime);
            audioClips.get(audioClips.size() - 1).play();   //play counter
            Thread.sleep(sleepTime);
            audioClips.get(s.getCounter()).play(); //play counter number
        } catch (Exception ex) {
    ...
    private class CounterListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if(!q.isEmpty()){
                customer = q.poll();
                customer.setServeTime(new GregorianCalendar());
    ...
    private void jbtReportActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtReportActionPerformed
        ...
        str += "\n" + "Average waiting time     : " + (totlaWaitingTime/serviceList.size() + " s");
    ...
    private void jbtTakeNumberActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtTakeNumberActionPerformed
        ...
        customer = new Customer(nextNumber);

        q.add(customer); //to consider the size, prevent add to queue when full
